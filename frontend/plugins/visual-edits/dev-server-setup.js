      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="temple-admin" -c user.email="admin@divinetemple.org" add "${targetFile}"`);
            execSync(`git -c user.name="temple-admin" -c user.email="admin@divinetemple.org" commit -m "temple_edit_${timestamp}"`);
          } catch (gitError) {
